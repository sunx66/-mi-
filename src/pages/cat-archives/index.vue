  ...
  <view class="page-sub">
    ...
    <view class="custom-nav" :style="{ paddingTop: statusBarHeight + 'px' }">
      <view class="nav-content">
        ...
        <view class="nav-title">我的档案</view>
    ...
    <view class="list-wrap" :style="{ paddingTop: contentTop + 'px' }">
      ...
      <view v-else-if="cats.length === 0" class="state-block">
        ...
        <text class="state-text">暂未创建猫咪档案</text>
      </view>
        ...
        >
          ...
          <view class="cat-info">
            <view class="cat-header">
              ...
              <view class="cat-status" :style="{ backgroundColor: getStatusColor(cat.status).bg, color: getStatusColor(cat.status).color }">
                <text class="status-text">{{ cat.statusText || '健康' }}</text>
              </view>
            </view>
            <text class="cat-meta">{{ cat.color }} · {{ cat.gender === 'male' ? '公' : cat.gender === 'female' ? '母' : '未知' }}</text>
            <text class="cat-location">📍 {{ cat.location }}</text>
            <text class="cat-time">建档：{{ formatDate(cat.createTime) }}</text>
          </view>
...
const cats = computed(() => {
  return catStore.catList
    .filter(c => c.createBy === userStore.userInfo.id)
    .slice().sort((a, b) => new Date(b.createTime) - new Date(a.createTime))
})
